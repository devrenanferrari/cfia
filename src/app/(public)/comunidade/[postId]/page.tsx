...
export default async function PostPage({ params }: { params: Promise<{ postId: string }> }) {
  ...
  // Increment views
  await prisma.post.update({ where: { id: postId }, data: { views: { increment: 1 } } }).catch(() => {});
  ...
  const [existingConn, authorPostCount, authorXP, savedRecord, relatedPosts] = await Promise.all([
    session && session.user.id !== post.author.id
      ? prisma.connection.findFirst({
          where: { OR: [{ fromId: session.user.id, toId: post.author.id }, { fromId: post.author.id, toId: session.user.id }] },
          select: { status: true },
        })
      : null,
    ...
    session
      ? prisma.savedPost.findUnique({ where: { userId_postId: { userId: session.user.id, postId: post.id } } })
      : null,
    post.tags.length > 0
      ? prisma.post.findMany({
          where: { id: { not: post.id }, tags: { hasSome: post.tags } },
          take: 4,
          orderBy: { createdAt: "desc" },
          select: { id: true, title: true, tags: true, _count: { select: { comments: true } } },
        })
      : [],
  ]);

  const connectionStatus = existingConn
    ? existingConn.status === "ACCEPTED" ? "accepted" : "pending"
    : "none";
  ...
  return (
    <div style={{ backgroundColor: "#f0f2f5", minHeight: "100vh" }}>
      <div className="mx-auto max-w-[1100px] px-4 py-6">
        <Link href="/comunidade" className="inline-flex items-center gap-2 text-sm mb-5 transition-colors hover:text-[#0f62fe]" style={{ color: "#8d8d8d" }}>
            ...
            <div className="bg-white border border-[#e0e0e0]">
              ...
              <div className="flex items-center justify-between gap-3 px-5 pt-5 pb-3">
                <Link href={`/u/${post.author.id}`} className="flex items-center gap-3 group min-w-0">
                  <div className="h-11 w-11 rounded-full flex items-center justify-center font-bold text-base shrink-0 group-hover:opacity-90 transition-opacity" style={{ backgroundColor: "#0f62fe", color: "#fff" }}>
                    {initials(post.author.name)}
                  </div>
                  <div className="min-w-0">
                    <p className="text-sm font-bold group-hover:text-[#0f62fe] transition-colors truncate" style={{ color: "#161616" }}>
                    ...
                    <p className="text-xs flex items-center gap-1.5 mt-0.5" style={{ color: "#8d8d8d" }}>
                      ...
                      {post.views > 0 && <span>· {post.views} visualizaç{post.views === 1 ? "ão" : "ões"}</span>}
                    </p>
                  </div>
                </Link>
                {session && session.user.id !== post.author.id && (
              ...
              <div className="px-5 pb-4">
                <h1 className="text-xl font-bold mb-4 leading-snug" style={{ color: "#161616" }}>
                  {post.title}
                </h1>
              ...
              {(likeCount > 0 || post._count.comments > 0) && (
                <div className="flex items-center gap-3 px-5 py-2 border-t border-[#f4f4f4] text-xs" style={{ color: "#8d8d8d" }}>
                  ...
                  {post._count.comments > 0 && (
                    <span className="ml-auto">{post._count.comments} comentário{post._count.comments !== 1 ? "s" : ""}</span>
                  )}
                </div>
              )}
              ...
              <PostActionBar
                ...
                isOwn={session?.user?.id === post.author.id}
              />
            </div>

            {/* Comentários */}
            <div className="bg-white border border-[#e0e0e0] p-5">
              <h2 className="text-sm font-bold mb-5 flex items-center gap-2" style={{ color: "#161616" }}>
          ...
          <aside className="lg:col-span-4 space-y-3">
            ...
            <div className="bg-white border border-[#e0e0e0] overflow-hidden">
              <div className="h-14" style={{ backgroundColor: "#0f62fe" }}>
                <div className="h-full opacity-10" style={{ backgroundImage: "repeating-linear-gradient(-45deg, #fff 0, #fff 1px, transparent 0, transparent 50%)", backgroundSize: "10px 10px" }} />
              </div>
              <div className="px-4 pb-4">
                <Link href={`/u/${post.author.id}`} className="block group -mt-6 mb-3">
                  <div className="h-12 w-12 rounded-full border-4 border-white flex items-center justify-center font-bold text-base group-hover:opacity-90 transition-opacity" style={{ backgroundColor: "#161616", color: "#fff" }}>
                    {initials(post.author.name)}
                  </div>
                </Link>
                <Link href={`/u/${post.author.id}`} className="font-bold text-sm hover:text-[#0f62fe] transition-colors inline-block" style={{ color: "#161616" }}>
                  {post.author.name ?? "Anônimo"}
                </Link>
                ...
                {post.author.bio && (
                  <p className="text-xs leading-relaxed mb-3" style={{ color: "#525252" }}>{post.author.bio}</p>
                )}
            ...
            {relatedPosts.length > 0 && (
              <div className="bg-white border border-[#e0e0e0] p-4">
                <p className="text-[10px] uppercase tracking-widest font-mono mb-3" style={{ color: "#8d8d8d" }}>Posts relacionados</p>
                <div className="space-y-3">
                  {relatedPosts.map((rp) => (
                    <Link key={rp.id} href={`/comunidade/${rp.id}`} className="block group">
                      <p className="text-xs font-bold leading-snug group-hover:text-[#0f62fe] transition-colors" style={{ color: "#161616" }}>
                        {rp.title}
                      </p>
                      <p className="text-[10px] mt-1" style={{ color: "#8d8d8d" }}>
                        {rp._count.comments} comentário{rp._count.comments !== 1 ? "s" : ""}
                      </p>
                    </Link>
                  ))}
                </div>
            ...
            <div className="bg-white border border-[#e0e0e0] p-4">
              ...
              <div className="space-y-0.5">
                {[{ href: "/comunidade", label: "Feed da comunidade", icon: Users }, { href: "/trilhas", label: "Trilhas de carreira", icon: TrendingUp }].map(({ href, label, icon: Icon }) => (
                  <Link key={href} href={href} className="flex items-center gap-2 text-xs px-2 py-2 hover:bg-[#f4f4f4] transition-colors" style={{ color: "#525252" }}>
                    <Icon className="h-3 w-3 shrink-0" style={{ color: "#0f62fe" }} /> {label}
                  </Link>
                ))}
              </div>
            </div>
