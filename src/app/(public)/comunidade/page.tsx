...
import {
  PenLine, Users, Zap, BookOpen, TrendingUp, ArrowRight, Search,
  Flame, Clock,
} from "lucide-react";
...
export default async function ComunidadePage({
  ...
  /* helpers para preservar outros params ao trocar sort */
          ...
          <aside className="lg:col-span-3 hidden lg:flex flex-col gap-3">
            ...
            <div className="bg-white border border-[#e0e0e0] overflow-hidden">
              {session ? (
                <>
                  <div className="relative h-20" style={{ backgroundColor: "#0f62fe" }}>
                    ...
                    <Link href={`/u/${session.user.id}`}>
                      <div
                        className="absolute -bottom-6 left-4 h-12 w-12 rounded-full border-4 border-white flex items-center justify-center font-bold text-base select-none hover:opacity-90 transition-opacity"
                  ...
                  <div className="pt-8 px-4 pb-4">
                    ...
                    <div className="mt-3 mb-3">
                      <div className="flex justify-between mb-1">
                        <span className="text-[11px] font-mono font-semibold" style={{ color: "#0f62fe" }}>
                          Nível {level}
                        </span>
                    ...
                    {/* Stats */}
                    <div
                      className="grid grid-cols-2 gap-px border rounded-sm overflow-hidden mb-3"
                    ...
                    <div className="space-y-0.5 border-t pt-3" style={{ borderColor: "#f4f4f4" }}>
                      ...
                      <Link
                        ...
                        style={{ color: "#525252" }}
                      >
                        <BookOpen className="h-3.5 w-3.5 shrink-0" style={{ color: "#0f62fe" }} /> Meus cursos
                      </Link>
                      <Link
                        href="/trilhas"
                        className="flex items-center gap-2 text-xs py-1.5 hover:text-[#0f62fe] transition-colors"
                        style={{ color: "#525252" }}
                      >
                        <TrendingUp className="h-3.5 w-3.5 shrink-0" style={{ color: "#0f62fe" }} /> Trilhas de carreira
                      </Link>
                    </div>
                  </div>
                </>
                ...
                <div className="p-4">
                  <div
                    className="h-12 w-12 rounded-full flex items-center justify-center mb-3 font-bold text-lg"
          ...
          <main className="lg:col-span-6 flex flex-col gap-0 sm:gap-3">
            ...
            <form method="GET" action="/comunidade" className="bg-white border-y sm:border border-[#e0e0e0]">
              <div className="flex items-center gap-2 px-4 py-2.5 border-b border-[#f4f4f4]">
                ...
                />
                {q && (
                  <Link href="/comunidade" className="text-xs hover:underline" style={{ color: "#8d8d8d" }}>
                    Limpar
                  </Link>
                )}
              </div>
            ...
            {/* Tabs de ordenação */}
            <div className="flex items-center bg-white border-y sm:border border-[#e0e0e0] overflow-hidden">
              <Link
                ...
                style={{
                  color: activeSort === "recent" ? "#0f62fe" : "#525252",
                  borderColor: activeSort === "recent" ? "#0f62fe" : "transparent",
                  backgroundColor: activeSort === "recent" ? "#f0f7ff" : "transparent",
                }}
              >
              ...
              <Link
                ...
                className="flex flex-1 items-center justify-center gap-1.5 h-10 text-xs font-bold transition-colors border-b-2"
                style={{
                  color: activeSort === "popular" ? "#0f62fe" : "#525252",
                  borderColor: activeSort === "popular" ? "#0f62fe" : "transparent",
                  backgroundColor: activeSort === "popular" ? "#f0f7ff" : "transparent",
                }}
              >
            ...
            {(q || tag) && (
              <div
                className="flex items-center gap-2 px-4 py-2 text-xs bg-white border-y sm:border border-[#e0e0e0]"
                style={{ color: "#525252" }}
              >
                <span>
                ...
                <Link href="/comunidade" className="ml-auto font-semibold hover:underline" style={{ color: "#0f62fe" }}>
                  Limpar filtros
                </Link>
              </div>
            )}
            ...
            {posts.length === 0 ? (
              <div className="bg-white border border-[#e0e0e0] p-12 text-center">
                ...
                <p className="text-sm" style={{ color: "#8d8d8d" }}>
                  {q || tag
                    ? "Tente outros termos ou navegue pelo feed."
            ...
            <div className="bg-white border border-[#e0e0e0] p-4">
              ...
              <div className="space-y-2.5">
                <div className="flex items-center justify-between text-xs">
                  ...
                  <span className="font-bold" style={{ color: "#161616" }}>
                ...
                <div className="space-y-3">
                  {suggestions.map((u) => (
                    <div key={u.id} className="flex items-center gap-3">
                      <Link href={`/u/${u.id}`}>
                        <div
                          className="h-9 w-9 rounded-full flex items-center justify-center text-sm font-bold shrink-0 select-none hover:opacity-90 transition-opacity"
                      ...
                      <div className="flex-1 min-w-0">
                        <Link
                          href={`/u/${u.id}`}
                          className="text-xs font-bold truncate block hover:text-[#0f62fe] transition-colors"
                          style={{ color: "#161616" }}
                        >
                          {u.name}
                        </Link>
                        <p className="text-[10px]" style={{ color: "#8d8d8d" }}>
                          ...
                          {u._count.posts > 0
                            ? ` · ${u._count.posts} post${u._count.posts !== 1 ? "s" : ""}`
                            : ""}
                        </p>
                      </div>
