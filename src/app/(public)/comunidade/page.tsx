...
import { PenLine, Users, Zap, BookOpen, TrendingUp, ArrowRight, Search } from "lucide-react";
      ...
      <div className="mx-auto max-w-[1200px] px-0 sm:px-4 py-0 sm:py-6">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-0 sm:gap-4">
              ...
              {session ? (
                <>
                  ...
                  <div className="relative h-20" style={{ backgroundColor: "#0f62fe" }}>
                    ...
                    />
                    <div
                      className="absolute -bottom-6 left-4 h-12 w-12 rounded-full border-4 border-white flex items-center justify-center font-bold text-base select-none"
                  ...
                  <div className="pt-8 px-4 pb-4">
                    ...
                    <div className="space-y-0.5 border-t pt-3" style={{ borderColor: "#f4f4f4" }}>
                      <Link href="/dashboard/cursos" className="flex items-center gap-2 text-xs py-1.5 hover:text-[#0f62fe] transition-colors" style={{ color: "#525252" }}>
                        <BookOpen className="h-3.5 w-3.5 shrink-0" style={{ color: "#0f62fe" }} /> Meus cursos
                      </Link>
                      <Link href="/trilhas" className="flex items-center gap-2 text-xs py-1.5 hover:text-[#0f62fe] transition-colors" style={{ color: "#525252" }}>
          ...
          <main className="lg:col-span-6 flex flex-col gap-0 sm:gap-3">
            ...
            {(q || tag) && (
              <div className="flex items-center gap-2 px-4 py-2 text-xs" style={{ color: "#525252" }}>
                <span>
                  ...
                  {suggestions.map((u) => (
                    <div key={u.id} className="flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-bold truncate" style={{ color: "#161616" }}>{u.name}</p>
              ...
              <ol className="space-y-2 text-xs" style={{ color: "#525252" }}>
                {["Respeito acima de tudo", "Foco em programação e IA", "Sem spam ou autopromoção", "Compartilhe conteúdo real"].map((r, i) => (
                  <li key={i} className="flex gap-2.5 items-start">
                    <span className="font-mono font-bold shrink-0" style={{ color: "#0f62fe" }}>{String(i + 1).padStart(2, "0")}</span>
                    {r}
                  </li>
                ))}
              </ol>
